...
@app.get("/async_example", response_class=HTMLResponse)
async def async_example(test: str = Depends(log_test)):
    logger.info(f"whatap_loguru_1.3.4:/async_example {test}")
    start_time = datetime.now()
    while True:
        if datetime.now() - start_time > timedelta(seconds=1):
            break
